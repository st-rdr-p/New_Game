...
namespace GameCore.Tests
{
    ...
    public class Vector3Tests
    {
        ...
        [Test]
        public void Vector3_CardinalDirections()
        {
        ...
        [Test]
        public void Vector3_Operations_Combined()
        {
            // Test combinations of operations
            var a = new Vector3(1, 2, 3);
            var b = new Vector3(2, 3, 4);
            
            // (a + b) * 2 - a = ((1+2), (2+3), (3+4)) * 2 - (1, 2, 3)
            // = (3, 5, 7) * 2 - (1, 2, 3) = (6, 10, 14) - (1, 2, 3) = (5, 8, 11)
            var result = (a + b) * 2 - a;
            var expected = new Vector3(5, 8, 11);
            Assert.That(result, Is.EqualTo(expected));
        }

        [Test]
        public void Vector3_Distance_Zero()
        {
            // Distance from point to itself is zero
            var vec = new Vector3(5, 10, 15);
            Assert.That(Vector3.Distance(vec, vec), Is.EqualTo(0));
        }

        [Test]
        public void Vector3_Lerp_Intermediate()
        {
            // Test 25% and 75% interpolation
            var start = new Vector3(0, 0, 0);
            var end = new Vector3(100, 100, 100);
            
            var quarter = Vector3.Lerp(start, end, 0.25f);
            Assert.That(quarter.X, Is.EqualTo(25));
            
            var threeQuarters = Vector3.Lerp(start, end, 0.75f);
            Assert.That(threeQuarters.X, Is.EqualTo(75));
        }
    }
}
